...
CATALOG={
    ...
    'pilot_pro_first_month':{'code':'pilot_pro_first_month','name':'첫 달 Pro 모집','suggested_amount_krw':PILOT_AMOUNT_KRW,
        'price_note':'첫 달 VAT 포함 39,000원 제안. 다음 달은 견적에 고정된 정상 Pro 월 요금이며 별도 자동 갱신 동의가 필요합니다.',
        'scope':'신규 구독 조직에 첫 달 Pro 1,000크레딧·3좌석을 제안합니다. 수락만으로 활성화되지 않으며 검증된 수납 후 지급합니다.'},
}
for item in CATALOG.values():item.update(automatic_renewal=False,credit_grant=0,checkout_enabled=False)
CATALOG['pilot_pro_first_month'].update(automatic_renewal=True,credit_grant=1000)
