...
app.conf.update(timezone='UTC', task_acks_late=True, worker_prefetch_multiplier=1,
    task_time_limit=120, beat_schedule={'review-jobs': {'task': 'phoenix.review_jobs', 'schedule': 5.0}})
...
def review_jobs():
    ...
    try:
        return process_pending_jobs(sessions, build_storage(settings), limit=2)
    finally:
        engine.dispose()
