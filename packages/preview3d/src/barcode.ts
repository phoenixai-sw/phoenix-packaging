...
export function ean13Geometry(value: string, moduleMm = .33, barHeightMm = 22.85) {
  ...
  if (!Number.isFinite(moduleMm)||moduleMm<.264||moduleMm>.66||!Number.isFinite(barHeightMm)||barHeightMm+.001<22.85*moduleMm/.33||barHeightMm>100) throw new Error("바코드 모듈 폭 또는 높이가 허용 범위를 벗어났습니다.");
