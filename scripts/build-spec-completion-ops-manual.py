"""Create the Korean manual from verified, local UI evidence only.
...
of the immutable platform PDFs. Release/deployment status is deliberately absent.
"""
...
c.setSubject("2026-09-18 격리 로컬 UI의 실제 수행 기록. 제조 승인 또는 운영 배포 확인 아님.")
ASSETS: set[Path] = set()
...
def start(page: int, title: str, subtitle: str):
    ...
    text("운영 도구와 출력 / 실제 사용 매뉴얼", M + 172, 30, 9, color=MUTED)
    text(title, M, 65, 24, True)
    para(subtitle, M, 102, CW, size=10, leading=15, color=MUTED)
    c.setStrokeColor(LINE)
    c.line(M, 42, W - M, 42)
    text("2026-09-18 · 격리 로컬 UI 검증 · 제조 승인 아님", M, H - 31, 8.3, color=MUTED)
    c.setFont("KR", 8.5)
    c.setFillColor(MUTED)
    c.drawRightString(W - M, 23, f"{page:02d} / 10")
...
manifest = {
    ...
    "pages": 10,
    "scope": "actual isolated local UI; no production deployment or manufacturing approval claim",
...
print(json.dumps({"path": str(TARGET), "pages": 10, "bytes": manifest["bytes"],
