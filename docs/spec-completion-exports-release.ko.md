...
2026-09-18 작성. 이 문서는 2차 개발 후보의 **실제 로컬 실행 기록**이다. 1차 편집 보완은 PR #7로 운영 배포됐다. 이 2차 변경의 운영 배포는 별도 검증 후 기록한다. 전체 작업지시서 완료나 최종 제조본 완성을 뜻하지 않는다.
...
- 비용·AI·맥락 집중 회귀 50개 통과. 서버 전체 회귀는 최종 결과 갱신 예정.
- 원본 ZIP 자체 21개를 포함한 집중 회귀 43개 통과. 다른 조직·팀 권한, 보관 상태, 파일 변조·한도·재시도·늦은 worker도 검사.
- 운영 반영 전 백업에서 41테이블·290행·32객체 복원을 확인했다. PostgreSQL 물리 복원/PITR 시험으로 부르지 않는다.
