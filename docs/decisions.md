...
- 비밀번호·Resend/SMTP 인증/재설정을 폐기하고 Google Identity Services + 서버 검증으로 전환한다. 관리자·운영팀은 ADMIN_EMAILS, 고객 작업 공간은 기존 ACL로 구분한다.
- 토스 연동 명세는 등록한 `tosspayments-integration-guide` MCP로 실제 조회한다. aibridge-site 구현을 현재 서버에 포팅하되 원장·주문금액·조직 격리 검증을 유지한다.
- 제조사 요청문을 폐기하고 GS1·ISO 공개 설명과 국내 3곳 공식 입고 규격을 조사한다. 공개 조건의 기본 검토 프로필과 실제 제작 승인 증거를 구분한다.
- 이전 검증 기록은 당시 증빙으로 보존한다. 오프라인 백업 복구는 격리 테스트 세션으로 앱 데이터를 재확인하며 실제 Google 로그인 검증으로 표시하지 않는다.
