...
실제 로컬 UI에서 자체 시험 SVG를 업로드해 **1200×900px → 90×67.5mm, X70/Y140mm**로 비율을 유지해 배치·저장했다. 저장본4에서 무료 편집 ZIP `0e658b5f…`도 완료됐으며, UI에 이미지2개(정화 SVG 원본과 표시 PNG)·글꼴2개·약5.9MiB가 표시됐다. 이 ZIP의 개별 파일 해시 검수는 앞의 글꼴 ZIP 검수와 별도다.
...
최종 소스 기준 전체 프런트 **78개**, 타입 검사, Scene/OpenAPI/TS 계약 drift가 통과했다. 과거 manifest·승인 철회 계약10개도 통과했다. 글꼴 API15개, 글꼴 포함 geometry216개, 지표25개와 관련 인증/결제/AI27개는 각 영역의 시험이며 겹치는 수를 합쳐 전체 서버 인수 결과로 쓰지 않는다. 새 후보의 공개 HTTP 계약은141개(JSON135·파일6)다. 전체 서버 회귀와 최종 production 빌드는 CI에서 확정할 항목이다.
...
마지막 원문 보완 중 검토 상태 전이와 완료 파일의 현재 승인 철회 안내는 코드·자동시험으로 확인했다. 검토 상태의 실제 메뉴 점검, 최종 전체 회귀·빌드·배포는 최종 보고에서 각각 갱신한다. 현재 기록만으로 운영 제공이나 원문 전체 완성을 선언하지 않는다. [전체 추적표](spec-completion-tracker.ko.md), [운영 전 조건](production-readiness.md)
