...
| AC31 · 백업 복구 | 암호화 백업을 별도 SQLite 메모리 DB로 논리 복원한 실제 증빙: 9개 표·23행·4개 객체, 값/FK/해시 일치. 작업 전 범위이며 새 전체 데이터의 PostgreSQL 별도 인스턴스 복구·로그인·장면/파일 재열기까지 통과한 증빙은 아니다. |
...
- 이미지 제공자 단가/품질/지연·제작 시간 절감·반려율의 실제 측정. A의 모델별 generation/edit HTTP 검증은 유료 공급자 실호출이나 상업적 품질 검증을 의미하지 않는다. 일회성 마케팅 이미지·영상 제작도 앱 내 생성·정산 검증과 별도로 기록한다.
