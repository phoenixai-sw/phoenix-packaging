# 편집·복구 1차 보완 — 사용 방법과 배포 검증

2026-09-18. 원문 작업지시서에서 누락됐던 편집·복구 기능을 보완한 편집 보완 기록이다. **1차 편집 보완은 2026-09-18 운영 배포 완료**: [PR #7](https://github.com/phoenixai-sw/phoenix-packaging/pull/7), 운영 커밋 `49e7c27`. Vercel 웹·API 배포 성공과 운영 관리자 로그인 세션에서 0크레딧·편집권·6개 저장본 비교를 확인했다. 기존 디자인은 변경하지 않았다. 이후 ZIP·등록 구조·비용 보호 변경은 별도 개발 중이며 이 배포에 포함하지 않는다. 기존 Google 운영 로그인과 실제 AI 연동은 이미 검증됐으며, 이번 편집 보완의 검증과 구분한다. 제조 출력·실제 결제·운영 관리까지 포함한 전체 개발 완료 보고가 아니다. 전체 남은 범위는 [요구사항 추적표](spec-completion-tracker.ko.md)에 있다.
...
2026-09-18 운영 Supabase에 `0008_editor_sessions`를 적용했다. 프로젝트13·리비전54·자산17·작업24개를 보존했고 새 편집권 테이블 RLS를 확인했다. 이행 전 암호화 백업은 40테이블289행32파일의 해시를 검증하고, 별도 로컬 DB와 파일 저장소로 복원한 뒤 앱으로 열었다. 운영 PostgreSQL 물리 복원/PITR 시험이나 실제 Google 재인증 시험과는 다르다.

운영 QA 프로젝트에서 기존 저장본5와6의 실제 위치·정렬 변경2건을 조회했다. 새 메뉴와 편집권·0크레딧을 확인했으며 운영 디자인 복원을 실행하지 않았다.

![운영 사이트의 실제 저장본 비교](images/editor-completion/production-revisions.png)
