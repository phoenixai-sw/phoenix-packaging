...
- 외부 인증 설정은 `GOOGLE_CLIENT_ID` 하나다. 클라이언트 비밀키나 메일 서버는 필요하지 않다.
- 운영 관리자와 운영 팀 계정은 서버의 `ADMIN_EMAILS` 쉼표 목록으로 정한다. 고객의 개인/팀 작업 공간은 기존 소유자·편집자·뷰어 권한으로 격리한다.
- 기존 aibridge-site 로컬 설정의 Google Client ID와 이메일 목록을 재사용한다. 값은 Git에 기록하지 않는다.
- Google Cloud의 **Google 인증 플랫폼 → 클라이언트 → 해당 웹 클라이언트 → 승인된 JavaScript 원본**에 `https://phoenix-packaging.vercel.app`이 있어야 한다. 로컬 검증에는 `http://localhost:3000`과 `http://127.0.0.1:3000`을 사용한다. 기존 서비스의 원본은 유지한다.
