...
### 4-0. 첫 번째로 생각할 문제 — 경쟁사 Packify와 **전개도 문제가 진짜 문제인가**

> 사용자 질문: "경쟁 상대로 생각하는 기존 사이트 **패키파이**를 확인해 보자. 니가 그렇게 말하는 **인쇄소 전개도가 정말 우리가 심각하게 생각할 문제인지** 말이야."

**경쟁사: https://www.packify.ai/** (사용자가 알려준 주소)

**배경.** 개발방-2의 Claude(나)는 인쇄 의뢰본을 뽑을 때마다 "칼선 치수는 승인된 도면이 아니니 이 파일로 발주하면 안 된다"를 반복했다. 사용자는 그것이 **실제 사업상 장애물인지, 아니면 Claude가 과하게 몸을 사린 것인지** 물었다. 정당한 문제 제기다.

#### 2026-09-24에 직접 확인한 것 (공개 페이지 기준)

| 항목 | Packify | Phoenix |
| --- | --- | --- |
| 칼선 제공 | **"Print-ready dielines — Quick templates"**, Business 요금제부터 `Editable` 내보내기 | 등록 구조가 있어야 나옴 |
| 칼선 근거 | **템플릿**. "From mockups to manufacturable dielines, we deliver" | 데모 기하 유도 또는 등록 구조 |
| 제조 연결 | `/print`은 **공급업체 디렉터리** — 지도 기반 업체 목록, 업체가 직접 등록 신청("I want to be included"). **인쇄를 대행하지 않는다** | 없음 |
| AI 1건 비용 | **10 크레딧** (이미지 1장당 약 10) | **10 크레딧** — 동일 |
| 무료 체험 | 100 크레딧 1회 | 가입 체험 30 크레딧 |
...
| Basic | $14.9 | 1,000 |
| Pro | $19.9 | 2,500 |
| **Business** | **$29.9** | 3,500 · **칼선 내보내기 여기부터** |
| Enterprise | $69.9 | 10,000 |

Phoenix는 **₩330,000 / 550,000 / 990,000** (월, VAT 포함)이고 Starter가 500 크레딧이다. **환율을 어떻게 잡아도 Packify Enterprise($69.9)보다 Phoenix Starter가 서너 배 비싸고, 크레딧은 오히려 적다.** 이것이 전개도보다 더 급한 문제일 수 있다.

그 밖에 Packify가 가진 것: AI 포토슈트, 로고 삽입, 라벨 생성기, **영양성분표 생성기**, API, 사내 디자이너 20명 이상의 유료 디자인 서비스. "200,000+ brands"는 마케팅 문구이므로 그대로 믿지 말 것.

#### 이 조사가 시사하는 답

**경쟁사는 승인된 제조사 도면을 요구하지 않는다.** 표준 템플릿으로 칼선을 주고, 제조는 디렉터리로 넘긴다. 즉 **치수 책임은 고객↔공급업체 사이에 있고, 플랫폼은 파일을 막지 않는다.**

따라서 §4-0의 선택지 중 **(B) 표준 규격을 우리가 미리 등록해 두는 쪽**이 시장 관행에 맞는다. 다행히 **지금 구조로 코드 수정 없이 가능하다**(§3-2). 흔한 스탠드 파우치·삼방 실링 규격 몇 개를 등록 구조로 넣어 두면 고객은 고르기만 하면 된다.

다만 다음 담당자가 **확인하고 판단할 것**:
1. Packify의 칼선이 주로 **박스·메일러(골판지)** 인지 — 그쪽은 규격이 표준화돼 있어 쉽다. **거싯 있는 파우치**는 설비 편차가 커서 같은 논리가 통하는지 별도로 봐야 한다.
2. 우리 `manufacturer_approval: false` 표기와 "PRINT REQUEST" 제목은 **유지하는 편이 낫다.** 파일을 막지 않되 사실은 남기는 것이 정직하고, 분쟁 시 우리를 보호한다.
3. **요금제를 재검토할 것.** 기능 경쟁보다 가격 격차가 더 큰 위험으로 보인다. 사용자와 먼저 상의하라.

**이미 확보한 사실(다시 조사하지 말 것).** 우리가 내보내는 인쇄 의뢰본은 실제 ICC로 CMYK 변환되고, 글꼴은 윤곽선이며, CUT/FOLD가 분리되고, 도련 3mm와 TrimBox가 정확하다. **기술적으로 부족한 것이 아니라 치수의 출처가 제조사가 아니라는 점만 남아 있다.**
